...
def test_fetch_and_to_db(database_obj):
    with database_obj.connect() as db:
        with patch.object(Date, "set_date_id") as mock_set_date_id, patch.object(
            Time, "set_time_id"
        ) as mock_set_time_id, patch.object(
            StockDataHandler, "insert_or_update_stock_data"
        ) as mock_insert_or_update_stock_data, patch.object(
            StockDataHandler, "insert_or_update_stock_data_ft"
        ) as mock_insert_or_update_stock_data_ft:
...
def test_insert_or_update_stock_data_ft(database_obj):
    with patch("pandas.read_sql") as mock_read_sql, patch(
