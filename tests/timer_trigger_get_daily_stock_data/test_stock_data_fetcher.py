...
def test_fetch(fetcher_instance):
    with patch.object(
        fetcher_instance, "scrape_top_stocks"
    ) as mock_scrape, patch.object(
        fetcher_instance, "clean_top_stocks"
    ) as mock_clean_top, patch.object(
        fetcher_instance, "get_stock_data"
    ) as mock_get_data, patch.object(
        fetcher_instance, "clean_stock_data"
    ) as mock_clean_data:

        mock_scrape.return_value = pd.DataFrame({"Symbol": ["AAPL", "MSFT"]})
        mock_clean_top.return_value = pd.Series(["AAPL", "MSFT"])
        mock_get_data.return_value = pd.DataFrame(
            {"symbol": ["AAPL", "MSFT"], "companyName": ["Apple", "Microsoft"]}
        )
        mock_clean_data.return_value = pd.DataFrame(
            {"symbol": ["AAPL", "MSFT"], "company_name": ["Apple", "Microsoft"]}
        )
...
def test_get_stock_data(fetcher_instance):
    symbols = ["AAPL", "MSFT"]
    with patch.object(
        fetcher_instance, "get_stock_info"
    ) as mock_get_info, patch.object(
        fetcher_instance, "get_newest_stock_prices"
    ) as mock_get_prices:
    ...
    expected_df = pd.DataFrame(
        {
            ...
            "industry": ["Tech", "Tech"],
            "datetime": [
                datetime.fromtimestamp(1622217600),
                datetime.fromtimestamp(1622217600),
            ],
            "close": [150.0, 250.0],
            "high": [155.0, 255.0],
