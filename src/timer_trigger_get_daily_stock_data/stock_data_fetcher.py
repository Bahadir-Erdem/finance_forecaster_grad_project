...
class StockDataFetcher:
    ...
    def clean_top_stocks(self, top_stocks: pd.DataFrame):
        ...
        top_stocks["% Change"] = pd.to_numeric(
            top_stocks["% Change"].str.replace("%", ""), errors="coerce"
        ).fillna(0)
        ...
        top_stocks[["Market Cap", "Revenue"]] = top_stocks[
            ["Market Cap", "Revenue"]
        ].map(lambda x: x.rstrip("B") if type(x) is str else x)
        top_stocks["Revenue"] = top_stocks["Revenue"].str.replace("-", "0")
        top_stocks.loc[top_stocks["Revenue"].str.endswith("M"), "Revenue"] = "39.48"
        top_stocks.loc[:, ["Market Cap", "Stock Price", "Revenue"]] = top_stocks[
            ["Market Cap", "Stock Price", "Revenue"]
        ].astype("float")
    ...
    def get_stock_data(self, symbols):
        ...
        for symbol in symbols:
            ...
            if any(
                dict_var is None for dict_var in (stock_info_dict, stock_price_dict)
            ):
                pass
            else:
